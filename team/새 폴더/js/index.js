...
function updateWidthTrasnform() {
  ...
  let slideTransform = -(slideWidth + slideMargin) * slideCount;

  console.log(slidesWrap)
  current_slide.forEach(slide => {
    slide.style.width = slideWidth +'px'
  })
  slides.style.width = (slideWidth + slideMargin) * newSlide_count +'px'
  ...
  if(window.innerWidth < 1024) {
    ...
    if(window.innerWidth < 768) {
      slideWidth = (slidesWrap - responsiveMargin/10) ;
      slideTransform = -(slideWidth + responsiveMargin) * slideCount;
  
      current_slide.forEach(slide => {
        slide.style.width = slideWidth +'px'
      })
      slides.style.width = (slideWidth + responsiveMargin) * newSlide_count +'px'
      slides.style.transform = `translateX(${slideTransform}px)`
    }else {
...
prevBtn.addEventListener("click", (e)=>{
  e.preventDefault()
  moveSlide(currentIdx - 1, slideWidth)
})
...
function moveSlide(num, slideWidth) {
